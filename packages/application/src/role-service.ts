...
export class RoleService {
  ...
  async assignRole(input: {
    ...
    if (!(await this.roles.roleExists(input.communityId, input.roleId))) {
      throw new NotFoundError("Role");
    }

    if (!(await this.roles.memberExists(input.communityId, input.targetUserId))) {
      throw new NotFoundError("Community member");
    }
    ...
    const authorized = authorizeRoleAssignment(
      {
        communityId: input.communityId,
        actorUserId: input.actorUserId,
        actorHighestRolePriority: actorPriority,
        actorPermissions: permissions
      },
      PERMISSIONS.ASSIGN_ROLES,
      { communityId: input.communityId, priority: targetPriority }
    );
    ...
    await this.auditLogs.write({
      ...
      metadata: { roleId: input.roleId, expiresAt: input.expiresAt?.toISOString() ?? null }
    });
  }
}
