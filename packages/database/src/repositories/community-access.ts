...
export function createCommunityAccessRepository(db: any) {
  return {
    async listForTelegramUser(telegramUserId: string) {
      const rows = await db.select({ memberId: communityMembers.id, id: communities.id, name: communities.title })
      ...
      return Promise.all(rows.map(async (row: any) => ({ id: row.id, name: row.name, roleName: await highestRole(db, row.memberId) })));
    },
    ...
    async getForTelegramUser(telegramUserId: string, communityId: string) {
      ...
      return { id: rows[0].id, name: rows[0].name, roleName: await highestRole(db, rows[0].memberId) };
    }
  };
}
